...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        ...
        if value.len() < 8 || header != Self::STANDARD_HEADER {
            return Err(PngError::ChunkHeaderError);
        }
        ...
        loop {
            let chunk_len = u32::from_be_bytes(
                chunks[0..4]
                    .try_into()
                    .expect("Chunk data type not correct"),
            ) as usize;

            let (left, right) = chunks.split_at(chunk_len + 12);
            let new_chunk = Chunk::try_from(left).expect("Chunk destruct error");
